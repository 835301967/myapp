...
func NewUnaryClientInterceptor(o ...ClientOption) grpc.UnaryClientInterceptor {
	...
	return func(
		...
		if span != nil {
			defer span.ReportAndEnd(metrics.RPCHistogramTracing, metrics.RPCSummaryTracing, map[string]string{
				"method": method,
			})
		}
		err := invoker(ctx, method, req, resp, cc, opts...)
		codeText := ykerrcode.ToRPCCodeText(ykerrcode.FromError(err).Code())
		metrics.InternalAccessTracing.With(prometheus.Labels{"method": method, "code": codeText}).Inc()
		return err
	}
}
